...
export function joinChannelHintText(): string {
  return 'Нажмите кнопку ниже, чтобы подписаться на канал.';
}

export function formatMarathonDate(isoUtc: string): string {
  try {
...
export function channelOpenedText(
  channelLink: string,
  refUrl = 'https://content2go.app/refH4kGr6DM',
  marathonDate = 'послезавтра в 10:00 (МСК)',
): string {
  return (
    `🎉 <b>Поздравляем! Вы зарегистрированы на практикум!</b>\n\n` +
    `Марафон стартует <b>${marathonDate}</b>. Это понятные инструкции и простые задания — выполнив их, вы уже разберётесь, как создавать контент-заводы для себя и для других за хорошие чеки.\n\n` +
    `✅ Выполняйте задания и получайте баллы. Потом их можно обменять на подарки от нашей ` +
    `<a href="${refUrl}">платформы</a> и на доступ в коммьюнити креаторов, которые уже успешно зарабатывают на контент-заводах.\n\n` +
    `📅 Все задания практикума будут выходить на основном канале платформы ${channelLink} и будут отправляться вам в этом боте\n\n` +
